...
(() => {
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      const gridMarkup = '<div class="preview-grid-heading"><h3 contenteditable="true" data-preview-field="heading" style="color:' + escape(textColor) + ';font-size:' + escape(headingSize) + 'px">' + escape(heading) + '</h3>' + extraElements.map((element, extraIndex) => element.type === 'heading' ? '<h3 contenteditable="true" data-preview-extra="' + extraIndex + '" style="color:' + escape(element.color || textColor) + ';font-size:' + escape(element.size || headingSize) + 'px;padding:' + escape(element.padding || 0) + 'px;margin:' + escape(element.margin || 0) + 'px">' + escape(element.text) + '</h3>' : '<p contenteditable="true" data-preview-extra="' + extraIndex + '" style="color:' + escape(element.color || descriptionColor) + ';font-size:' + escape(element.size || descriptionSize) + 'px;padding:' + escape(element.padding || 0) + 'px;margin:' + escape(element.margin || 0) + 'px">' + escape(element.text) + '</p>').join('') + '<p contenteditable="true" data-preview-field="content" style="color:' + escape(descriptionColor) + ';font-size:' + escape(descriptionSize) + 'px">' + escape(text) + '</p></div><div class="preview-feature-grid" style="grid-template-columns:repeat(' + gridColumns + ', minmax(0,1fr));gap:' + escape(getValue(card, 'grid_gap') || '24') + 'px">' + blocks.map(block => '<div class="preview-feature ' + (getValue(card, 'card_layout') === 'icon_left' ? 'is-icon-left' : 'is-stacked') + '" style="text-align:' + escape(getValue(card, 'card_alignment') || 'center') + '">' + (block.previewImage || block.image ? '<img src="' + escape(block.previewImage || ('/' + String(block.image).replace(/^\//, ''))) + '" alt="">' : '<b>' + escape(block.icon || '✦') + '</b>') + '<div><h4>' + escape(block.title || 'Feature title') + '</h4>' + (block.text ? '<p>' + escape(block.text) + '</p>' : '') + '</div></div>').join('') + '</div>';
      const textExtraMarkup = type === 'text' ? extraElements.map((element, extraIndex) => element.type === 'heading' ? '<h3 contenteditable="true" data-preview-extra="' + extraIndex + '" style="color:' + escape(element.color || textColor) + ';font-size:' + escape(element.size || headingSize) + 'px;padding:' + escape(element.padding || 0) + 'px;margin:' + escape(element.margin || 0) + 'px">' + escape(element.text) + '</h3>' : '<p contenteditable="true" data-preview-extra="' + extraIndex + '" style="color:' + escape(element.color || descriptionColor) + ';font-size:' + escape(element.size || descriptionSize) + 'px;padding:' + escape(element.padding || 0) + 'px;margin:' + escape(element.margin || 0) + 'px">' + escape(element.text) + '</p>').join('') : '';
  ...
  preview.querySelector('.live-preview-content').addEventListener('blur', (event) => {
    if (event.target.dataset.previewField || event.target.dataset.previewExtra !== undefined) render();
  }, true);
