...
(() => {
  ...
  function renderStyles(card, target = card.dataset.selectedTarget || 'section') {
    ...
    const extraElementControls = ['feature_grid', 'text'].includes(type) ? '<div class="block-editor"><label>Extra section elements</label>' + extraElements.map((element, index) => {
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      const textExtraMarkup = type === 'text' ? extraElements.map((element, extraIndex) => previewExtraElement(element, extraIndex, textColor, headingSize, descriptionColor, descriptionSize)).join('') : '';
