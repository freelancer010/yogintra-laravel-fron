...
(() => {
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      if (type === 'custom_columns' && blocks.length > gridColumns) { const grid = previewSection.querySelector('.preview-custom-columns'); blocks.slice(gridColumns).forEach((block, index) => { const blockIndex = gridColumns + index; const column = document.createElement('div'); column.className = 'preview-feature is-stacked preview-column-stack'; column.dataset.previewColumnTarget = 'column-' + blockIndex + '-container'; column.style.gridColumn = 'span ' + Math.min(gridColumns, Number(block.column_span || 1)); column.style.textAlign = textAlign; column.innerHTML = block.type === 'image' ? (block.previewImage || block.image ? '<img data-preview-column-image="' + blockIndex + '" data-preview-column-target="column-' + blockIndex + '-image" src="' + escape(block.previewImage || ('/' + String(block.image).replace(/^\//, ''))) + '" alt="">' : '<button type="button" class="preview-image-empty" data-preview-empty-image="' + blockIndex + '">Add an image</button>') : (block.type === 'button' ? '<span class="preview-cta" data-preview-column-target="column-' + blockIndex + '-button">' + escape(block.button_text || 'Button label') + '</span>' : '<div><h4 contenteditable="true" data-preview-column="' + blockIndex + '" data-preview-column-key="title" data-preview-column-target="column-' + blockIndex + '-title">' + escape(block.title || 'Add a heading') + '</h4><p contenteditable="true" data-preview-column="' + blockIndex + '" data-preview-column-key="text" data-preview-column-target="column-' + blockIndex + '-text">' + escape(block.text || 'Add text for this row.') + '</p></div>'); grid.appendChild(column); }); }
      ...
      if (type === 'feature_grid') {
        ...
        const addLabel = gridColumns < 3 ? '＋ Add column' : '＋ Add row';
        const addHint = gridColumns < 3 ? 'Add a new column' : 'Add a new full-width row';
        sectionActions.innerHTML = '<button type="button" class="preview-section-add" data-quick-add-trigger>' + addLabel + '</button><div class="preview-section-add-menu"><small>' + addHint + '</small><button type="button" data-quick-add-column="text">Text</button><button type="button" data-quick-add-column="image">Image</button><button type="button" data-quick-add-column="button">Button</button></div>';
      ...
      if (sectionActions.innerHTML) {
        sectionActions.addEventListener('click', event => {
          ...
          if (columnChoice) {
            ...
            if (gridColumns >= 3) blocks.push({ type: columnChoice.dataset.quickAddColumn, title: '', text: '', column_span: 3 });
            else { while (blocks.length < nextCount) blocks.push({ type: 'empty', title: '', text: '' }); blocks[nextCount - 1] = { ...blocks[nextCount - 1], type: columnChoice.dataset.quickAddColumn }; }
            getField(card, 'blocks').value = JSON.stringify(blocks);
            getField(card, 'grid_columns').value = String(gridColumns >= 3 ? gridColumns : nextCount); focusCard(card, 'section'); render(); return;
          }
