...
(() => {
  ...
  function renderStyles(card, target = card.dataset.selectedTarget || 'section') {
    ...
    stylePanel.querySelectorAll('[data-block]').forEach(control => {
      const updateBlock = () => { blocks[control.dataset.block][control.dataset.blockKey] = control.value; blocksField.value = JSON.stringify(blocks); renderStyles(card, target); render(); };
      control.addEventListener('input', updateBlock); control.addEventListener('change', updateBlock);
    });
