...
(() => {
  ...
  const focusCard = (card, target = 'section') => {
    ...
    const selectedLabel = columnTarget
      ? ({ title: 'Heading', text: 'Text', small_text: 'Supporting text', bullets: 'Bullet list', image: 'Image', button: 'Button', testimonial: 'Testimonial', container: 'Column' })[columnTarget[1]]
  ...
  function renderStyles(card, target = card.dataset.selectedTarget || 'section') {
    ...
    const columnTarget = type === 'custom_columns' && /^column-(\d+)-(title|text|small_text|bullets|image|button|testimonial|container)$/.exec(target);
    if (columnTarget) {
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      if (type === 'feature_grid') {
        ...
        sectionActions.innerHTML = '<button type="button" class="preview-section-add" data-quick-add-trigger>＋ Add element</button><div class="preview-section-add-menu"><small>Add to this section</small>' + (gridColumns < 3 ? '<button type="button" data-quick-add-column="text">Text column</button><button type="button" data-quick-add-column="image">Image column</button><button type="button" data-quick-add-column="button">Button column</button>' : '') + '<button type="button" data-quick-add-testimonial>Testimonial</button></div>';
      ...
      if (sectionActions.innerHTML) {
        sectionActions.addEventListener('click', event => {
          ...
          if (event.target.closest('[data-quick-add-testimonial]')) {
            blocks.push({ type: 'testimonial', image: '', rating: 5, review: 'YogIntra made my wellness journey feel supported, calm and genuinely personal.' });
            getField(card, 'blocks').value = JSON.stringify(blocks); focusCard(card, 'column-' + (blocks.length - 1) + '-testimonial'); render(); queueSnapshot(); return;
          }
