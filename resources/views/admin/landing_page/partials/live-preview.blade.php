...
(() => {
  ...
  picker.addEventListener('click', event => {
    ...
    setTimeout(() => {
      ...
      if (field && ['image', 'feature_grid'].includes(type)) { if (!field.querySelector('option[value="' + type + '"]')) field.add(new Option(type === 'feature_grid' ? 'Feature grid' : 'Image / Hero', type)); field.value = type; if (type === 'image') ensureField(card, 'image_size', '100').value = '100'; field.dispatchEvent(new Event('change', { bubbles: true })); }
  ...
  function renderStyles(card, target = card.dataset.selectedTarget || 'section') {
    ...
    if (type === 'feature_grid' && !blocks.length) { blocks = [{ icon: '☼', title: 'Yoga for Health', text: 'Describe the benefit.' }, { icon: '♡', title: 'Yoga for Cardio', text: 'Describe the benefit.' }, { icon: '◒', title: 'Yoga for Body', text: 'Describe the benefit.' }, { icon: '◌', title: 'Yoga for Breathing', text: 'Describe the benefit.' }]; blocksField.value = JSON.stringify(blocks); }
    const blockControls = type === 'feature_grid' ? '<div class="block-editor"><label>Feature blocks</label>' + blocks.map((block, index) => '<div class="feature-block-control"><input data-block="' + index + '" data-block-key="icon" value="' + escape(block.icon || '') + '" maxlength="4" aria-label="Icon"><input data-block="' + index + '" data-block-key="title" value="' + escape(block.title || '') + '" aria-label="Title"><textarea data-block="' + index + '" data-block-key="text" aria-label="Description">' + escape(block.text || '') + '</textarea><button type="button" data-remove-block="' + index + '">Remove</button></div>').join('') + '<button type="button" class="add-feature-block">+ Add block</button></div>' : '';
    ...
    stylePanel.innerHTML = '<div class="builder-inspector-title"><span>' + targetLabel + ' design</span><span>◐</span></div><button type="button" class="delete-selected-section">Delete section</button>' + (target === 'section' ? imageControl + blockControls : '') + elementControls + formatControls + (target === 'section' ? '<button type="button" class="apply-section-spacing">Apply this spacing to all sections</button>' : '');
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      const gridMarkup = '<div class="preview-grid-heading"><h3 contenteditable="true" data-preview-field="heading" style="color:' + escape(textColor) + ';font-size:' + escape(headingSize) + 'px">' + escape(heading) + '</h3><p contenteditable="true" data-preview-field="content" style="color:' + escape(descriptionColor) + ';font-size:' + escape(descriptionSize) + 'px">' + escape(text) + '</p></div><div class="preview-feature-grid">' + blocks.map(block => '<div class="preview-feature"><b>' + escape(block.icon || '✦') + '</b><div><h4>' + escape(block.title || 'Feature title') + '</h4><p>' + escape(block.text || '') + '</p></div></div>').join('') + '</div>';
      ...
      layer.innerHTML = '<button type="button" class="section-layer"><span>☷ ' + escape(treeLabel) + '</span><small>' + escape(type.replace('_', ' + ')) + '</small></button><div class="section-tree-children"></div>';
