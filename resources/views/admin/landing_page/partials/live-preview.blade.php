...
(() => {
  ...
  picker.addEventListener('click', event => {
    ...
    const sourceType = ['image', 'feature_grid', 'trainer_slider', 'custom_columns', 'testimonial', 'faq'].includes(resolvedType) ? 'image_text' : resolvedType;
    ...
    setTimeout(() => {
      ...
      if (field && ['image', 'feature_grid', 'trainer_slider', 'custom_columns', 'testimonial', 'faq', 'image_text', 'cta'].includes(resolvedType)) {
        ...
        if (resolvedType === 'feature_grid' || resolvedType === 'trainer_slider') {
          ...
          blocks.value = JSON.stringify(resolvedType === 'trainer_slider'
            ? [
              { title: 'Trainer name', location: 'Area served', text: 'Add a short trainer introduction.', image: '' },
              { title: 'Trainer name', location: 'Area served', text: 'Add a short trainer introduction.', image: '' },
              { title: 'Trainer name', location: 'Area served', text: 'Add a short trainer introduction.', image: '' }
            ]
            : (type === 'services' && serviceBlocks.length ? serviceBlocks : [{ icon: '✚', title: 'Traditional Healing', text: '' }, { icon: '♨', title: 'Improve Health', text: '' }, { icon: '☯', title: 'Holistic Wellness', text: '' }]));
  ...
  function renderStyles(card, target = card.dataset.selectedTarget || 'section') {
    ...
    const blockControls = (type === 'feature_grid' || type === 'trainer_slider') ? (type === 'trainer_slider'
      ? '<div class="block-editor trainer-block-editor"><p class="builder-control-help">Each slide has a photo, trainer name, location served, and optional introduction.</p><label>Trainer cards</label>' + blocks.map((block, index) => '<div class="feature-block-control trainer-block-control"><div class="feature-block-image">' + (block.previewImage || block.image ? '<img src="' + escape(block.previewImage || ('/' + String(block.image).replace(/^\//, ''))) + '" alt="">' : '<b>♙</b>') + '<button type="button" data-block-image="' + index + '">' + (block.image ? 'Replace image' : 'Add image') + '</button></div><input data-block="' + index + '" data-block-key="title" value="' + escape(block.title || '') + '" aria-label="Trainer name" placeholder="Trainer name"><input data-block="' + index + '" data-block-key="location" value="' + escape(block.location || '') + '" aria-label="Location served" placeholder="Location served"><textarea data-block="' + index + '" data-block-key="text" aria-label="Trainer introduction" placeholder="Short introduction">' + escape(block.text || '') + '</textarea><button type="button" data-remove-block="' + index + '">Remove trainer</button></div>').join('') + '<button type="button" class="add-feature-block">+ Add trainer</button></div>'
    ...
    stylePanel.querySelector('.add-feature-block')?.addEventListener('click', () => { blocks.push(type === 'trainer_slider' ? { title: 'Trainer name', location: 'Area served', text: 'Add a short trainer introduction.', image: '' } : { icon: '✦', title: 'New feature', text: 'Describe the benefit.' }); blocksField.value = JSON.stringify(blocks); renderStyles(card); render(); });
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      if (type === 'feature_grid' || type === 'trainer_slider') {
        sectionActions.innerHTML = '<button type="button" class="preview-section-add" data-quick-add-feature>＋ Add ' + (type === 'trainer_slider' ? 'trainer' : 'feature card') + '</button>';
