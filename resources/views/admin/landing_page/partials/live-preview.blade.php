...
(() => {
  ...
  function renderStyles(card, target = card.dataset.selectedTarget || 'section') {
    ...
    const columnTarget = type === 'custom_columns' && /^column-(\d+)-(title|text|small_text|bullets|image|button)$/.exec(target);
    if (columnTarget) {
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      if (type === 'custom_columns') blocks.slice(0, gridColumns).forEach((block, index) => { if (block.type === 'button') previewSection.querySelectorAll('.preview-column-stack')[index]?.querySelector('.preview-cta')?.setAttribute('data-preview-column-target', 'column-' + index + '-button'); });
