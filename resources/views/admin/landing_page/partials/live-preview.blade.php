...
(() => {
  ...
  function render() {
    ...
    cards.forEach((card, index) => {
      ...
      if (backgroundMode === 'image' && backgroundImage) { const rgb = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(overlayColor); const overlay = rgb ? 'rgba(' + parseInt(rgb[1], 16) + ',' + parseInt(rgb[2], 16) + ',' + parseInt(rgb[3], 16) + ',' + overlayOpacity + ')' : 'rgba(0,0,0,0)'; previewSection.style.backgroundImage = 'linear-gradient(' + overlay + ',' + overlay + '), url("' + backgroundImage.replace(/"/g, '%22') + '")'; previewSection.style.backgroundSize = 'cover'; previewSection.style.backgroundRepeat = 'no-repeat'; previewSection.style.backgroundPosition = (getValue(card, 'background_position') || 'center') + ' center'; }
      ...
      if (type === 'feature_grid') {
        ...
        sectionActions.innerHTML = '<button type="button" class="preview-section-add" data-quick-add-trigger>＋ Add element</button><div class="preview-section-add-menu"><small>Add a new column</small><button type="button" data-quick-add-column="text">Text</button><button type="button" data-quick-add-column="image">Image</button><button type="button" data-quick-add-column="button">Button</button></div>';
      ...
      if (sectionActions.innerHTML) {
        sectionActions.addEventListener('click', event => {
          ...
          if (columnChoice) {
            ...
            while (blocks.length < nextCount) blocks.push({ type: 'text', title: '', text: '' });
