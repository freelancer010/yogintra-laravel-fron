...
@if($page_sections->isNotEmpty())
  <style>
    ...
    /* The opening introduction follows the hero as a compact, editorial
       section rather than repeating hero-scale type and spacing. */
    .landing-builder-section.landing-intro-section { padding-top:54px !important; padding-bottom:54px !important; background:#f7f4ee !important; }
    .landing-intro-section .row { justify-content:center; }
    .landing-intro-section h2 { max-width:820px; margin:0 auto 18px !important; color:#0d3f45 !important; font-family:Roboto,Arial,sans-serif !important; font-size:clamp(34px, 3.65vw, 52px) !important; font-weight:700 !important; line-height:1.12 !important; letter-spacing:-.025em; }
    .landing-intro-section h3 { display:table; margin:0 auto 16px !important; padding:7px 14px; border-radius:999px; background:#dcefee; color:#0d6772 !important; font-family:Roboto,Arial,sans-serif !important; font-size:12px !important; font-weight:700 !important; letter-spacing:.1em; line-height:1.2 !important; text-transform:uppercase; }
    .landing-intro-section .landing-builder-content { max-width:760px; color:#355c66 !important; font-family:Roboto,Arial,sans-serif; font-size:18px !important; line-height:1.65; }
    .landing-intro-section .landing-builder-content p + p { margin-top:16px; }
    .landing-intro-section .landing-section-button,
    .landing-intro-section .btn.btn-theme-colored { border-color:#0d6772 !important; background:#0d6772 !important; color:#fff !important; font-family:Roboto,Arial,sans-serif !important; font-weight:700 !important; }
    .landing-intro-section .landing-section-button:hover,
    .landing-intro-section .btn.btn-theme-colored:hover { border-color:#094f56 !important; background:#094f56 !important; }
    ...
    .landing-builder-section h3 { font-family:Roboto,Arial,sans-serif !important; font-weight:700 !important; color:#01aeb7 !important; line-height:1.08 !important; }
    ...
    .landing-builder-section .landing-trainer-card h3 { margin:0; padding:18px 16px 4px; color:#183c45; font-size:20px; font-weight:700; }
    .landing-builder-section .landing-trainer-location { display:flex; align-items:center; justify-content:center; gap:7px; margin:0; padding:0 16px 10px; color:#0d6772; font-size:14px; font-weight:700; line-height:1.45; }
    .landing-builder-section .landing-trainer-location i { font-size:13px; }
    .landing-builder-section .landing-trainer-bio { margin:0; padding:0 16px 18px; color:#647b82; line-height:1.55; }
          ...
          <div class="landing-trainer-slider" data-nav="true" data-dots="true">
            @forelse($trainers as $trainer)
              <div class="landing-trainer-slide"><article class="landing-trainer-card">
                ...
                @if(!empty($trainer['location']))<p class="landing-trainer-location"><i class="fas fa-map-marker-alt" aria-hidden="true"></i>{{ $trainer['location'] }}</p>@endif
                @if(!empty($trainer['text']))<p class="landing-trainer-bio">{{ $trainer['text'] }}</p>@endif
              </article></div>
            @empty
          ...
          <div class="landing-custom-columns" style="--column-count: {{ $columnCount }}; text-align: {{ $section->text_align ?? 'left' }};">
            @foreach(array_slice($columns, 0, $columnCount) as $column)
              ...
              @php
                ...
                $columnStyle = function ($key, $defaultColor, $defaultSize, $element = null) use ($columnStyles, $section) {
                  $style = $element['styles'] ?? ($columnStyles[$key] ?? []);
                  $padding = (int) ($style['padding'] ?? 0);
                  $margin = (int) ($style['margin'] ?? 0);
                  return 'color: '.($style['color'] ?? $defaultColor).'; font-size: '.(int)($style['size'] ?? $defaultSize).'px; padding: '.(int)($style['padding_top'] ?? $padding).'px '.(int)($style['padding_right'] ?? $padding).'px '.(int)($style['padding_bottom'] ?? $padding).'px '.(int)($style['padding_left'] ?? $padding).'px; margin: '.(int)($style['margin_top'] ?? $margin).'px '.(int)($style['margin_right'] ?? $margin).'px '.(int)($style['margin_bottom'] ?? $margin).'px '.(int)($style['margin_left'] ?? $margin).'px; text-align: '.($style['align'] ?? $section->text_align ?? 'left').';';
                };
              ...
              <div class="landing-custom-column" style="display: flex; flex-direction: column; justify-content: {{ ['start' => 'flex-start', 'center' => 'center', 'end' => 'flex-end'][$column['vertical_align'] ?? 'start'] }}; padding: {{ (int) ($column['padding_y'] ?? 0) }}px {{ (int) ($column['padding_x'] ?? 0) }}px; margin: {{ (int) ($column['margin_y'] ?? 0) }}px {{ (int) ($column['margin_x'] ?? 0) }}px;">
                ...
                @elseif(($column['type'] ?? 'text') === 'button')
                  @php
                    ...
                    $buttonSpacing = ((int) ($buttonStyle['padding'] ?? 0) > 0 ? 'padding: '.(int) $buttonStyle['padding'].'px;' : '')
                      .((int) ($buttonStyle['margin'] ?? 0) > 0 ? ' margin: '.(int) $buttonStyle['margin'].'px;' : '');
                  @endphp
                  @if(!empty($column['button_text']) && !empty($column['button_url']))
                    <a href="{{ $column['button_url'] }}" class="btn btn-theme-colored btn-flat" style="{{ $buttonSpacing }}">{{ $column['button_text'] }}</a>
                  @endif
                @else
                ...
                @foreach(($column['elements'] ?? []) as $element)
                  @if(($element['type'] ?? '') === 'heading')
                    <h3 style="{{ $columnStyle('title', $section->text_color ?? '#183c45', 24, $element) }}">{!! app(\App\Support\HtmlSanitizer::class)->sanitize($element['text'] ?? '') !!}</h3>
                  @elseif(($element['type'] ?? '') === 'subtext')
                    <p class="landing-builder-content" style="{{ $columnStyle('text', $section->description_color ?? '#647b82', 16, $element) }}">{!! app(\App\Support\HtmlSanitizer::class)->sanitize($element['text'] ?? '') !!}</p>
                  @elseif(($element['type'] ?? '') === 'bullets')
                    @php
                      $stackBullets = array_filter(array_map('trim', preg_split('/\r?\n/', $element['items'] ?? '')));
                    @endphp
                    @if($stackBullets)<ul class="landing-column-bullets" style="{{ $columnStyle('bullets', $section->description_color ?? '#647b82', 16, $element) }}">@foreach($stackBullets as $bullet)<li>{{ $bullet }}</li>@endforeach</ul>@endif
        ...
        @elseif($section->section_type === 'feature_grid')
          ...
          <div class="mb-4" style="text-align: {{ $section->text_align ?? 'left' }};">
            ...
            @foreach($extraElements as $element)
              @php $elementPadding = $element['padding'] ?? 0; $elementMargin = $element['margin'] ?? 0; $elementSpacing = 'padding: '.($element['padding_top'] ?? $elementPadding).'px '.($element['padding_right'] ?? $elementPadding).'px '.($element['padding_bottom'] ?? $elementPadding).'px '.($element['padding_left'] ?? $elementPadding).'px; margin: '.($element['margin_top'] ?? $elementMargin).'px '.($element['margin_right'] ?? $elementMargin).'px '.($element['margin_bottom'] ?? $elementMargin).'px '.($element['margin_left'] ?? $elementMargin).'px;'; @endphp
              @if(($element['type'] ?? '') === 'heading')<h3 style="color: {{ $element['color'] ?? $section->text_color ?? '#183c45' }}; font-size: {{ $element['size'] ?? $section->heading_size ?? 32 }}px; {{ $elementSpacing }}">{!! app(\App\Support\HtmlSanitizer::class)->sanitize($element['text'] ?? '') !!}</h3>
              @elseif(in_array(($element['type'] ?? ''), ['bullet_list', 'numbered_list'], true))
                @php
                  $items = array_filter(array_map('trim', preg_split('/\r?\n/', $element['items'] ?? '')));
                @endphp
                @if(($element['type'] ?? '') === 'numbered_list')<ol class="landing-builder-list" style="color: {{ $element['color'] ?? $section->description_color ?? '#647b82' }}; font-size: {{ $element['size'] ?? $section->description_size ?? 16 }}px; {{ $elementSpacing }} --list-item-gap: {{ $element['item_gap'] ?? 8 }}px;">@foreach($items as $item)<li>{{ $item }}</li>@endforeach</ol>
                @else<ul class="landing-builder-list" style="color: {{ $element['color'] ?? $section->description_color ?? '#647b82' }}; font-size: {{ $element['size'] ?? $section->description_size ?? 16 }}px; {{ $elementSpacing }} --list-item-gap: {{ $element['item_gap'] ?? 8 }}px;">@foreach($items as $item)<li>{{ $item }}</li>@endforeach</ul>@endif
              @else<p style="color: {{ $element['color'] ?? $section->description_color ?? '#647b82' }}; font-size: {{ $element['size'] ?? $section->description_size ?? 16 }}px; {{ $elementSpacing }}">{!! app(\App\Support\HtmlSanitizer::class)->sanitize($element['text'] ?? '') !!}</p>@endif
            @endforeach
            @if($section->content)<div class="landing-builder-content" style="color: {{ $section->description_color ?? '#647b82' }}; font-size: {{ $section->description_size ?? 16 }}px; {{ $contentSpacing }}">{!! app(\App\Support\HtmlSanitizer::class)->sanitize($section->content) !!}</div>@endif
          </div>
