            ...
            @foreach(array_slice($columns, 0, $columnCount) as $column)
              ...
              <div class="landing-custom-column" style="padding: {{ (int) ($column['padding_y'] ?? 0) }}px {{ (int) ($column['padding_x'] ?? 0) }}px; margin: {{ (int) ($column['margin_y'] ?? 0) }}px {{ (int) ($column['margin_x'] ?? 0) }}px;">
                ...
                @elseif(($column['type'] ?? 'text') === 'button')
                  ...
                  @if(!empty($column['button_text']) && !empty($column['button_url']))<a href="{{ $column['button_url'] }}" class="btn btn-theme-colored btn-flat" style="{{ (int) ($buttonStyle['padding'] ?? 0) > 0 ? 'padding: '.(int) $buttonStyle['padding'].'px;' : '' }} {{ (int) ($buttonStyle['margin'] ?? 0) > 0 ? 'margin: '.(int) $buttonStyle['margin'].'px;' : '' }}">{{ $column['button_text'] }}</a>@endif
                @else
