...
class LandingPageController extends Controller
{
    ...
    private function upgradeClassicTemplateSections(int $pageId): bool
    {
        ...
        $storyHeading = 'Why YogIntra';
        $storyContent = '<p>YogIntra makes yoga easier to fit into real life. Choose online, at-home or personalized sessions shaped around your schedule, experience and goals.</p><p>Our approach is practical, welcoming and instructor-led—so you can build a steady practice that feels supportive from the first session.</p>';

        // Earlier Classic canvases contained two overlapping About sections.
        // Convert the first into the image-led brand story and remove only the
        // legacy logo-card counterpart, preserving any later editor changes.
        $legacyStory = LandingPageSection::where('landing_page_id', $pageId)
            ->where('heading', 'Start where you are. Practice at your pace.')
            ->first();
        if ($legacyStory) {
            $legacyStory->update([
                ...
                'image' => 'assets/landing-reference/guidance.webp',
                'image_alt' => 'Woman practising yoga',
                'image_position' => 'left',
                'image_size' => 40,
                'button_text' => 'Learn more about YogIntra',
                'button_url' => url('about'),
                'text_align' => 'left',
                'background_color' => '#ffffff',
                'text_color' => '#143b43',
                'description_color' => '#4d6c72',
                'padding_y' => 68,
            ]);
            LandingPageSection::where('landing_page_id', $pageId)->where('heading', $aboutHeading)->delete();
            $changed = true;
        }

        if (!LandingPageSection::where('landing_page_id', $pageId)->where('heading', $aboutHeading)->exists() && !LandingPageSection::where('landing_page_id', $pageId)->where('heading', $storyHeading)->exists()) {
        ...
        // Keep the Classic template intentionally calm: deep teal and a warm
        ...
        $classicHeadings = [
            ...
            'Start Your Yoga Journey in 3 Simple Steps',
            'Yoga Plans for Different Needs',
            'Frequently Asked Questions',
            'Ready to Start Your Yoga Journey?',
        ];
        foreach (LandingPageSection::where('landing_page_id', $pageId)->whereIn('heading', $classicHeadings)->get() as $section) {
            $isTealSection = $section->heading === 'Benefits of Regular Yoga Practice';
            $palette = [
                'background_color' => $isTealSection ? '#0d6772' : '#f7f4ee',
                'text_color' => $isTealSection ? '#ffffff' : '#0d6772',
                'description_color' => $isTealSection ? '#ffffff' : '#0d6772',
            ];
            if (array_intersect_assoc($palette, $section->only(array_keys($palette))) !== $palette) {
                $section->update($palette);
                $changed = true;
            }
        }
    ...
    private function createClassicSections(int $pageId, string $city, string $legacyContent = ''): void
    {
        ...
        $sections = [
            ...
            ['section_type' => 'image_text', 'heading' => 'Why YogIntra', 'content' => '<p>YogIntra makes yoga easier to fit into real life. Choose online, at-home or personalized sessions shaped around your schedule, experience and goals.</p><p>Our approach is practical, welcoming and instructor-led—so you can build a steady practice that feels supportive from the first session.</p>', 'image' => 'assets/landing-reference/guidance.webp', 'image_alt' => 'Instructor guiding a yoga student', 'image_position' => 'left', 'image_size' => 40, 'button_text' => 'Learn more about YogIntra', 'button_url' => url('about'), 'text_align' => 'left', 'background_color' => '#ffffff', 'text_color' => '#143b43', 'description_color' => '#4d6c72', 'padding_y' => 68],
            ...
            ['section_type' => 'feature_grid', 'heading' => 'Yoga Plans for Different Needs', 'content' => '<p>Choose a package based on your preferred schedule, class format and practice goals. Contact YogIntra for current pricing and availability.</p>', 'blocks' => $planBlocks, 'grid_columns' => 3, 'card_layout' => 'stacked', 'card_alignment' => 'left', 'text_align' => 'center', 'background_color' => '#f7f4ee', 'text_color' => '#0d6772', 'description_color' => '#0d6772', 'padding_y' => 72],
            ['section_type' => 'custom_columns', 'heading' => 'Frequently Asked Questions', 'content' => '', 'blocks' => $homepageFaqBlocks, 'grid_columns' => 1, 'text_align' => 'center', 'background_color' => '#f7f4ee', 'text_color' => '#0d6772', 'description_color' => '#0d6772', 'padding_y' => 72],
            ['section_type' => 'cta', 'heading' => 'Ready to Start Your Yoga Journey?', 'content' => '<p>Whether you are taking your first class or looking for a more consistent practice, YogIntra makes it easier to find yoga sessions that fit your lifestyle.</p>', 'button_text' => 'Book Your Yoga Session', 'button_url' => url('contact'), 'text_align' => 'center', 'background_color' => '#f7f4ee', 'text_color' => '#0d6772', 'description_color' => '#0d6772', 'padding_y' => 64],
        ];
